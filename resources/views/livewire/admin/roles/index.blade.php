<div wire:key="product-table" wire:init="loadData">
    ...
    <div class="card mb-3">
                ...
                <table class="table table-sm table-hover table-bordered fs-8 table-responsive scrollbar">
                    <thead class="bg-custom-navbar">
                        <tr>
                            ...
                            <th class="text-white" scope="col">Role ID</th>
                            <th class="text-white" scope="col">Name</th>
                            <th class="text-white" scope="col">Created</th>
                            <th class="text-white" scope="col">Created By</th>
                            <th class="text-white" scope="col">Updated</th>
                            ...
                            <td align="center">
                                <div class="position-static"><a class=" dropdown-toggle dropdown-caret-none transition-none btn-reveal" type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent"><span class="fas fa-cog fs-10"></span></a>
                                    <div class="dropdown-menu dropdown-menu-end py-2">
                                        <a onclick="crudJson('edit','{{ $data->role_id }}')" class="dropdown-item text-primary btnEdit"><i class="fa fa-eye"></i> View</a>
                                        <div class="dropdown-divider"></div>
                                        <a onclick="crudJson('delete','{{ $data->role_id }}')" class="dropdown-item text-danger btnDelete" href="#!"><i class="fa fa-trash"></i> Remove</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
        ...
        {{-- Modal untuk CRUD --}}
        @include('livewire.admin.roles.crud')
...
<script>
        ...
        if (action === 'create') {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let roledata = data[0];
                    let menudata = data[1];
                    $("#id").val(roledata.role_id);
                    $('#role_id').val(roledata.role_id);
                    $('#name_role').val(roledata.name_role);
                    $.each(menudata, function(index, item) {
                        $('#' + item.menu_id).prop('checked', item.is_actived == 1);
                    });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, result => {
                        $("#id").val(data[result].role_id);
                        $('#role_id').val(data[result].role_id);
                        $('#name_role').val(data[result].name_role);
                    })
    ...
    // Jika semua menu-checkbox dicentang/di-uncheck manual, update check_all
    ...
    $(document).on('submit', '#formSubmit', function(e) {
        ...
        $('.menu-checkbox').each(function() {
            ...
            if ($(this).is(':checked')) {
                menuData.push({
                    ...
                    is_actived: $(this).is(':checked') ? 1 : 0
                });
            }
        ...
        // Gabungkan menuData ke formData
        formData.push({
            name: 'menuData',
