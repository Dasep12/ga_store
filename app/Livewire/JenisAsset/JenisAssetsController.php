...
class JenisAssetsController extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        if ($this->isReady) {
            $datas = DB::table('tbl_mst_jenis_asset')
                ->where(function ($q) {
                    $q->where('name', 'like', '%' . $this->search . '%')
                        ->orWhere('kode_asset', 'like', '%' . $this->search . '%');
    ...
    {
        $data = DB::table('tbl_mst_jenis_asset')
            ->where('kode_asset', $id)
            ->first();
        ...
        switch ($request->crudAction) {
            case 'create':
                $validator = Validator::make($request->all(), [
                    ...
                    'kode_asset' => 'required|string|max:50|unique:tbl_mst_jenis_asset,kode_asset',
                    'is_actived' => 'boolean'
                ]);
            ...
            case 'edit':
                ...
                DB::table('tbl_mst_jenis_asset')
                    ->where('kode_asset', $request->id)
                    ->update([
                        'name' => $request->name,
                ...
                break;
            case 'delete':
                DB::table('tbl_mst_jenis_asset')->where('kode_asset', $request->id)->delete();
